...
real Calibre tools, databases, and background services. They test
...
class TestBookIngestInContainer:
    ...
    def test_ingest_epub_already_target_format(
        self,
        sample_ebook_path,
        ingest_folder,
        library_folder,
    ...
    def test_ingest_multiple_files(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        ...
        while time.time() - start_time < max_wait:
            ...
            if not remaining:
                ...
                break
            print(
                f"⏳ Waiting for ingest... {len(remaining)} files remaining ({int(time.time() - start_time)}s)"
            )
            time.sleep(3)
        ...
        if not all_processed:
            ...
            pytest.fail(
                f"Not all files processed within {max_wait} seconds. Remaining: {remaining_names}"
            )
        ...
        with sqlite3.connect(
            str(get_db_path(metadata_db, tmp_path)), timeout=30
        ) as con:
            cur = con.cursor()
            result = cur.execute("SELECT COUNT(*) FROM books").fetchone()
            book_count = result[0] if result else 0
...
class TestIngestBackups:
    ...
    def test_imported_files_backed_up(
        self, sample_ebook_path, ingest_folder, test_volumes, tmp_path, cwa_container
    ):
        """
...
class TestInternationalCharacters:
    ...
    def test_ingest_international_filename(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        """
        ...
        if dest.exists():
            pytest.fail(
                f"File with international characters was not processed within {max_wait} seconds"
            )
        ...
        print(
            f"✅ Book with international characters successfully imported (library has {book_count} book(s))"
        )
...
class TestFormatConversion:
    ...
    def test_mobi_to_epub_conversion(
        self, ingest_folder, library_folder, test_volumes, tmp_path, cwa_container
    ):
        """
        ...
        book_dirs = [
            d for d in library_folder.iterdir() if d.is_dir() and d.name != ".keep"
        ]
        ...
        if os.getenv("USE_DOCKER_VOLUMES", "false").lower() == "true":
            ...
            print(
                f"✅ MOBI successfully imported (verified via database and directory structure)"
            )
            return
        ...
        print(
            f"📊 Files found: {len(mobi_files_imported)} MOBI, {len(azw3_files)} AZW3, {len(azw_files)} AZW, {len(epub_files)} EPUB"
        )
        ...
        print(
            f"✅ MOBI successfully imported as {len(mobi_files_imported)} MOBI, {len(azw3_files)} AZW3, {len(epub_files)} EPUB"
        )

    def test_conversion_failure_moves_to_failed_folder(
        self, ingest_folder, test_volumes, tmp_path, cwa_container
    ):
        """
    ...
    def test_txt_to_epub_conversion(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        ...
        while dest_file.exists() and time.time() - start_time < max_wait:
            ...
            if int(time.time() - start_time) % 15 == 0:
                print(
                    f"⏳ Waiting for TXT conversion... ({int(time.time() - start_time)}s)"
                )
...
class TestProcessLockInContainer:
    ...
    def test_lock_released_after_processing(
        self, ingest_folder, sample_ebook_path, tmp_path, cwa_container
    ):
        """
...
class TestAdvancedIngestFeatures:
    ...
    def test_directory_import_processes_all_files(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        """
        ...
        while time.time() - start_time < max_wait:
            ...
            if len(remaining) == 0 and not sub_dir.exists():
                ...
                break
            print(
                f"⏳ Waiting... {len(remaining)} files remaining ({int(time.time() - start_time)}s)"
            )
            time.sleep(5)

        if not all_processed:
            pytest.skip(
                f"Directory processing may not be fully implemented or takes longer"
            )
...
class TestFilenameHandling:
    ...
    def test_filename_truncation_at_150_chars(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        """
    ...
    def test_empty_folder_cleanup_after_processing(
        self, ingest_folder, tmp_path, cwa_container
    ):
        """
        ...
        while (
            sub_dir / epub_path.name
        ).exists() and time.time() - start_time < max_wait:
            time.sleep(2)
        ...
        if not sub_dir.exists():
            ...
            print(
                "ℹ️  Directory still exists (may contain hidden files or cleanup not implemented)"
            )
...
class TestIngestStability:
    ...
    def test_processing_survives_multiple_files(
        self, ingest_folder, library_folder, tmp_path, cwa_container
    ):
        """
        ...
        for i in range(num_files):
            ...
            if dest.exists():
                pytest.fail(
                    f"File {i+1}/{num_files} not processed, ingest may have crashed"
                )
    ...
    def test_zero_byte_file_doesnt_crash_ingest(
        self, ingest_folder, tmp_path, cwa_container
    ):
        """
...
class TestMetadataAndDatabase:
    ...
    def test_book_appears_in_metadata_db(
        self,
        ingest_folder,
        library_folder,
        sample_ebook_path,
        ...
        while dest_file.exists() and time.time() - start_time < max_wait:
            ...
            if check_count % 5 == 0:  # Print every 10 seconds
                print(
                    f"⏳ Still waiting for file to be processed... ({time.time() - start_time:.1f}s elapsed)"
                )
            time.sleep(2)
        ...
        with sqlite3.connect(str(get_db_path(local_db, tmp_path)), timeout=30) as con:
            ...
            # Check book exists
            book = cur.execute(
                "SELECT id, title, path FROM books ORDER BY id DESC LIMIT 1"
            ).fetchone()
            assert book is not None, "No book record in metadata.db"

            book_id, title, path = book
            print(
                f"✅ Book in metadata.db: ID={book_id}, title='{title}', path='{path}'"
            )
    ...
    def test_cwa_db_tracks_import(
        self, ingest_folder, test_volumes, sample_ebook_path, tmp_path, cwa_container
    ):
        """
        ...
        if os.getenv("USE_DOCKER_VOLUMES", "false").lower() == "true":
            pytest.skip(
                "cwa.db access requires config volume (not available in DinD mode)"
            )
        ...
        with sqlite3.connect(str(get_db_path(cwa_db, tmp_path)), timeout=30) as con:
            ...
            tables = cur.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='cwa_import'"
            ).fetchone()
...
class TestRealWorldScenarios:
    ...
    def test_user_drops_book_and_it_appears_in_library(
        self, ingest_folder, library_folder, cwa_container, tmp_path
    ):
        """
        ...
        if fixtures_dir.exists():
            ...
            for epub in all_epubs:
                ...
                print(
                    f"  - {epub.name}: {size:,} bytes (test file: {starts_with_test})"
                )

        # Use a real book (not synthetic)
        real_books = [
            f
            for f in fixtures_dir.glob("*.epub")
            if not f.name.startswith("test_") and f.stat().st_size > 10000
        ...
        with sqlite3.connect(
            ...
            cur = con.cursor()
            books = cur.execute(
                "SELECT title FROM books ORDER BY timestamp DESC LIMIT 1"
            ).fetchone()
        ...
        book_dirs = [
            d
            for d in library_folder.iterdir()
    ...
    def test_mixed_format_batch_import(
        self, ingest_folder, library_folder, cwa_container, tmp_path
    ):
        """
        ...
        print(
            f"📚 Found {len(epub_files)} EPUB, {len(mobi_files)} MOBI, {len(txt_files)} TXT files"
        )
        ...
        processing_time = int(time.time() - start_time)
        print(
            f"✅ All {len(files_to_import)} files processed in {processing_time} seconds"
        )
